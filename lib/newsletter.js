...
// Cover paths may be site-relative ("/covers/x.jpg") or absolute (CDN/Wikipedia).
function coverSrc(cover) {
  return /^https?:\/\//.test(cover) ? cover : `${SITE_URL}${cover}`;
}

function renderNote(note) {
  const meta = [note.creator, note.year].filter(Boolean).map(escapeHtml).join(', ');
  const url = noteUrl(note);
  // The cover floats at the start of the body so only the body text wraps around
  // it (date + title stay full-width above). align="left" handles Outlook, which
  // ignores float; the table cell contains the float so it can't bleed into the
  // next note.
  const cover = note.cover
    ? `<a href="${url}"><img class="cover" src="${coverSrc(note.cover)}" alt="${escapeHtml(note.title)} cover" width="124" align="left" style="float:left;width:124px;max-width:124px;height:auto;border:1.5px solid ${INK};margin:2px 22px 10px 0;" /></a>`
    : '';
  ...
  return `
    <tr>
      <td class="rule" style="padding:40px 0;border-top:1px solid ${RULE};">
        ...
        <div class="note-content" style="font-family:${FONT_UI};font-size:17px;line-height:1.7;color:${INK};margin-top:20px;">${cover}${note.contentHtml || ''}</div>
      </td>
    </tr>`;
}
  ...
  <head>
    ...
    <style>
      ...
      @media (prefers-color-scheme: dark) {
        ...
        .rule, .cover { border-color: #3a3631 !important; }
