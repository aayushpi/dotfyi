...
// All valid types
export const NOTE_TYPES = ['book', 'film', 'tv', 'article', 'game', 'essay', 'thought'];

// Types that are "log" entries (have cover, creator, year, etc.)
export const LOG_TYPES = ['book', 'film', 'tv', 'article', 'game'];
...
function parseNoteFile(filename) {
  ...
  return {
    ...
    year: data.year instanceof Date ? data.year.getFullYear() : data.year != null ? String(data.year) : null,
...
// Returns note metadata with the full rendered HTML body (no link previews) for
// notes published strictly after `sinceIso`, sorted oldest-first. Used by the newsletter.
export async function getNotesAfter(sinceIso) {
  if (!fs.existsSync(notesDir)) return [];
  ...
  return notes
    .filter((n) => n.date && new Date(n.date).getTime() > since)
    .sort((a, b) => (a.date < b.date ? -1 : 1));
}

// Returns the ISO date of the earliest-dated note, or null. Used for the
// newsletter masthead ("… since <month year>").
export function getOldestNoteDate() {
  if (!fs.existsSync(notesDir)) return null;
  let oldest = null;
  for (const filename of fs.readdirSync(notesDir).filter((f) => f.endsWith('.md'))) {
    const { data } = matter(fs.readFileSync(path.join(notesDir, filename), 'utf8'));
